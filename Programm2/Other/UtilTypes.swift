...
struct Particle: _StaticDefaultProperty {
    ...
    var pos: float3
    /// position of particle in last frame (for vcerlet integration)
    var lastPos: float3
    ...
    var isLocked: Bool
    /// init of Particle
    init(pos: float3, lastPos: float3, force: float3, mass: Float, isLocked: Bool) {
        self.pos = pos
    ...
    init(x: Float, y: Float, z: Float, mass: Float, isLocked: Bool = false) {
        let pos = float3(x: x, y: y, z: z)
        self.init(pos: pos, lastPos: pos, force: float3(x: 0, y: 0, z: 0), mass: mass, isLocked: isLocked)
    }
...
struct CircularMeshParameters: CustomStringConvertible {
