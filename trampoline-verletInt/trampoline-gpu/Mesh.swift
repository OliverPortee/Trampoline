...
class HelperGraphics: Renderable {
    ...
    init(fromParameters parameters: CircularMeshParameters, device: MTLDevice, smoothness: Float = 20) {
...
class CircularTrampolineMesh: Mesh {
    ...
    /// meshParamters
    var parameters: CircularMeshParameters!
    ...
    func initParameters(_ parameters: CircularMeshParameters) {
...
extension CircularTrampolineMesh {
    ...
    class HelperParticle: Equatable {
        ...
        init(pos: float3, force: float3, mass: Float, isLocked: Bool) {
            self.pos = pos
            self.mass = mass
        ...
        convenience init(x: Float, y: Float, z: Float, mass: Float, isLocked: Bool) {
            self.init(pos: float3(x: x, y: y, z: z), force: float3(0, 0, 0), mass: mass, isLocked: isLocked)
        }
        
        static func == (lhs: HelperParticle, rhs: HelperParticle) -> Bool {
            return (lhs.pos == rhs.pos) && (lhs.mass == rhs.mass) && (lhs.isLocked == rhs.isLocked)
        }
        ...
        var particle: Particle {
            return Particle(pos: self.pos, lastPos: self.pos, force: float3(x: 0, y: 0, z: 0), mass: self.mass, isLocked: self.isLocked)
        }
    ...
    func makeCircularJumpingSheet(parameters: CircularMeshParameters) -> ([Particle], [Spring], [Float]) {
        // helper function
        ...
        for row in 0..<n_particles {
            for col in 0..<n_particles - 1 {
                ...
                connections.append(HelperSpring(p1: p1, p2: p2, springConstant: parameters.innerSpringConstant, velConstant: parameters.innerVelConstant, initialLength: parameters.fineness ))
