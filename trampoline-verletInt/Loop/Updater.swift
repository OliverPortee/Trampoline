...
class MeshUpdater: NSObject {
    ...
    */
    ...
    /// updates springs
    func updateSprings(dt: Float, mesh: Mesh) {
        ...
        let physicalUniforms = device.makeBuffer(bytes: [dt], length: MemoryLayout<Float>.stride, options: [])!
        computeCommandEncoder.setBuffer(physicalUniforms, offset: 0, index: BufferIndex.PhysicalUniformsBufferIndex.rawValue)
